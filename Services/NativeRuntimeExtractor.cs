...
public static class NativeRuntimeExtractor
{
    ...
    public static void EnsureExtracted()
    {
        ...
        lock (_lock)
        {
            ...
            // Check version — if already extracted with same version, skip
            ...
            foreach (var name in resourceNames)
            {
                ...
                // Remove prefix, keep subfolder structure (e.g. avx2/ggml-cpu.dll)
                var relativePath = name.Substring(prefix.Length);
                var fullPath = Path.Combine(_nativeDir, relativePath.Replace('/', Path.DirectorySeparatorChar));
