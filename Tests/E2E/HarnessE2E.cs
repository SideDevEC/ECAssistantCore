...
public sealed class HarnessE2E
{
        ...
        Action<string>? prepareWorkingDir = null)
    {
        ...
        var (session, dir) = await factory.CreateAsync(
            ...
            configure: engine =>
            {
                ...
                configureWithConfig?.Invoke(engine, config);
            },
            prepareWorkingDir: dir =>
            {
                // Tools resolve paths against AgentSettings.WorkingDirectory —
                // pin it to the isolated session dir so builds never touch the
                // test runner's current directory.
                config.AgentSettings.WorkingDirectory = dir;
                prepareWorkingDir?.Invoke(dir);
            });
        return (session, dir);
    }
    ...
    /// <summary>
    /// v14.20 journey — typed per-tool outputs, live: a REAL dotnet build fails,
    /// EDotnetBuildTool's RenderForModel compacts the log, and the model answers
    /// the failing file + error code from the semantic render alone. Only
    /// EDotnetBuild is registered → grammar union forces the right tool.
    /// </summary>
    [Fact]
    public async Task BuildFailureJourney_ModelReportsFileAndCode_FromCompactRender()
    {
        if (string.IsNullOrEmpty(ServerUrl)) return; // not enabled — needs a real server

        var (session, dir) = await CreateSessionAsync(ServerUrl, (engine, config) =>
            engine.RegisterTool(new EDotnetBuildTool(new ProcessRunner(), config)),
            prepareWorkingDir: dir =>
            {
                // A deliberately broken console project AT THE ROOT of the session
                // working dir — `dotnet build` (no projectPath) picks up the only
                // csproj. A subdir would need the model to pass projectPath.
                var projectDir = dir;
                File.WriteAllText(Path.Combine(projectDir, "BrokenApp.csproj"),
                    """
                ...
                File.WriteAllText(Path.Combine(projectDir, "Program.cs"),
                    """
                    var order = new Order();
                    System.Console.WriteLine(order.Totall); // CS1061: 'Order' has no 'Totall'
        ...
        try
        {
            ...
            var result = await session.Orchestrator.ExecuteMultiStep(
                "Run a dotnet build of BrokenApp.csproj in the working directory, then report the failing source file name and the compiler error code. Do not attempt to fix anything.");

            // The model must have answered from the compact render — it never
            // saw the raw MSBuild log in context.
            var buildVisible = string.Join("\n", session.Engine.ContextWindow.GetWindowMessages()
                .Where(m => m.Role == "tool_output" && m.Source == "EDotnetBuild")
                .Select(m => m.Content));
            Assert.True(buildVisible.Contains("[BUILD FAILED]"),
                $"expected compact render in model context, got: {buildVisible}");
            Assert.DoesNotContain(dir, buildVisible); // absolute path noise dropped live

            Assert.True(result.Status == OrchestratorStatus.GoalAchieved,
                $"expected GoalAchieved, got {result.Status}: {result.FinalOutput}");
            Assert.Contains("Program.cs", result.FinalOutput);
            Assert.Contains("CS1061", result.FinalOutput);
        }
        finally
        {
            await session.DisposeAsync();
