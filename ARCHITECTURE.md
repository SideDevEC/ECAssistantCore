...
- **Tier operating profiles (v15):** SMALL tier gets a dedicated, self-contained OS prompt file (`SystemPrompt.{Mac|Windows|Linux}.Small.md` — one tool call per turn, never-invent do-nots, worked envelope examples, ZERO conflicting messaging); LARGE tier gets the full original base prompt + slim autonomy overlay. Tool-level tier prompts enforced through `EToolBase.ToSystemPromptBlock(isLargeTier)` (hooks: `GetToolRulesForTier`/`GetToolExampleForTier`). system prompt carries a tier overlay — small tier: precision rules + worked examples + do-nots, one tool call per turn; large tier: autonomy profile (plan internally, batch, verify, minimal reporting). Reasoning effort (`llm_provider.reasoning_effort`, off/low/medium/high) flows to `reasoning_effort` on requests. Output-token budgets are NOT tier-dependent (config/hardware only — envelope budget removed).
