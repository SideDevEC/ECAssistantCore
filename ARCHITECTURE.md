...
**Addendum 2026-09-26 (handoff flake hardening — filler wording + test modes):** The neutral context-summary filler is now the terminal directive "No additional context. Give your final answer now." — earlier wordings invited tool-wandering on small models ("Complete the task you were created for" -> hunting last_prompt.txt; "Your system instructions define your task" -> hunting appsettings.json). The echo journey test instructs the parent to pass sampling:'greedy' + thinking:'off' (the agent-decided modes). Remaining known limitation (documented honestly): a 4B parent model does not reliably RELAY the flags/prompt verbatim through natural-language instruction — the definitive fix is a host-side RegisterHandoff-based test variant (pending owner decision).

**Addendum 2026-09-26 (thinking off the prompt — request param, owner-approved):** `InferenceRequestParams.Thinking` (bool?) rides the request like temperature, serialized as `thinking` in every chat-completions payload (HttpStreamingEngine, all 3 serializers). HandoffExecutor's `ResolveSpecialistParams` maps `thinking:"off"` to `Thinking=false` (param copy, parent never mutated) — and the `/no_think` prompt soft switch is REMOVED (no more model-dependent prompt text; the mechanism is now structured request state handled server-side: template prefill for Qwen3-family vocabs + stream-side ThinkFilter enforcement everywhere). greedy mapping unchanged.

**Addendum 2026-09-26 (specialist decision template — option 5, owner-approved):** Handoff/specialist engines are constructed with `isSpecialistSession: true` (`AgentEngine` ctor injection). Their first-turn decision prompt replaces the main agent's "Decide yourself" wrapper with a task-execution frame: *"The user message above is the task to execute… it is a directive, execute it. If the task's deliverable is text, finish now with that text as your answer. If the task needs tools, start working and call the first tool now."* — the "greetings/conversation → answer directly" branch is removed for specialists only; the main agent template is untouched. Motivation: the HandoffE2E flake where a literal `'none'` context summary became the specialist's opening message and a 4B model answered it conversationally (system-prompt contract lost to the wrapper's "decide yourself" framing). Also in `HandoffExecutor`: placeholder hygiene — context summaries of `none`/`-`/empty normalize to the neutral filler "Complete the task you were created for." (string normalization, no semantic analysis). Journey validation: HandoffE2E 2/2 pass on the live server with both changes verified in server logs (variant prompt + normalized opening message).
**Addendum 2026-09-26 (agent-decided specialist modes — options 2+4, owner-approved):** The delegating MODEL opts the specialist into sampling/thinking behavior per handoff call via the EHandoff envelope: new optional args `sampling:"greedy"` (specialist runs at temperature 0 → native greedy chain; for deterministic deliverables) and `thinking:"off"` (Qwen3 `/no_think` soft switch appended to the specialist's opening message — skips the think block server-side). Flow: envelope args → `HandoffRequest.SamplingStyle`/`.Thinking` → `HandoffExecutor` (greedy: `ResolveSpecialistParams` returns a COPY of the parent's request params with `Temperature=0` — parent's params object never mutated; no-think: soft switch appended to the opening message). No semantic analysis anywhere — pure structural opts the agent declares at delegation time. Default behavior unchanged (args omitted = inherited params, thinking on). Handoff unit 26/26, E2E 2/2.
...
- **Tier = THE process switch (v15):** preplanning (small: decompose-first, large: in-loop), run limiter (small: turns, large: `model_tier.timeout_seconds` wall-clock budget), inference values (`model_tier.inference` overrides global sampling per tier), prompts + tool blocks per tier. Removed redundant config: `interface.preplanning`, `interface.verify_command` (verification.build_command is the single verifier contract). **Shell compute (v15):** large tier gets a persistent shell session (cwd/env persist across tool calls — `PersistentShellSession`, sentinel protocol) executing inside a macOS Seatbelt sandbox (workspace+temp writes, network denied by default — `SeatbeltShellSandbox`; small tier keeps isolated host execution). Best-effort containment, not a security boundary. preplanning (small: decompose-first, large: in-loop), run limiter (small: turns, large: `model_tier.timeout_seconds` wall-clock budget), inference values (`model_tier.inference` overrides global sampling per tier), prompts + tool blocks per tier. Removed redundant config: `interface.preplanning`, `interface.verify_command` (verification.build_command is the single verifier contract). SMALL tier gets a dedicated, self-contained OS prompt file (`SystemPrompt.{Mac|Windows|Linux}.Small.md` — one tool call per turn, never-invent do-nots, worked envelope examples, ZERO conflicting messaging); LARGE tier gets the full original base prompt + slim autonomy overlay. Tool-level tier prompts enforced through `EToolBase.ToSystemPromptBlock(isLargeTier)` (hooks: `GetToolRulesForTier`/`GetToolExampleForTier`). system prompt carries a tier overlay — small tier: precision rules + worked examples + do-nots, one tool call per turn; large tier: autonomy profile (plan internally, batch, verify, minimal reporting). Reasoning effort (`llm_provider.reasoning_effort`, off/low/medium/high) flows to `reasoning_effort` on requests. Output-token budgets are NOT tier-dependent (config/hardware only — envelope budget removed).
...
- **Idle watchdog / stateless background tasks:** decompose/summarize use `HttpStreamingEngine` stateless mode (no session_id), same server. **v15 contract:** `SessionId=null` on request params is a REAL stateless signal — the engine must not fall back to a default session (legacy `?? _defaultSessionId` leaked decompose/planner/summary prompts into the main session's KV cache and pushed shift-incapable models over the context wall; 16k J2 failure). Omitted `session_id` → server stateless executor.
- **Server-truth compaction (v15):** before each turn, KV status is refreshed and the LARGER of window-estimate vs server `ApproxTokens` is compared against the SMALLER of window vs server context size — compaction can never fire late. Compaction input is capped to the window (`maxBudget·0.6·3` chars) so the summarize call itself cannot overflow at small windows; after a window self-summarize the server KV cache is reset + re-fed (window/KV desync fix). A server 413 `context_overflow` marks the model shift-incapable → compaction threshold 80%→60%, cache rebuild + single retry.
...
| IInferenceEngine | HttpStreamingEngine | LLM generation over HTTP (stream/generate → LLMDecision); v15: params.SessionId=null = stateless, never routed to a default session |
