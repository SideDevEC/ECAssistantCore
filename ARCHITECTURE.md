...
**Updated:** 2026-09-22 (evening — LIVE E2E HARNESS DAY + fix train: (1) REMOTE MODE CONTEXT FIX (82c5174): remote native function-calling calls now carry the static prefix (as system message + native-tools response-format override) and conversation history — remote providers have no server-side state, previously the model saw only the incremental turn fragment (goldfish behavior: clarify-loops, envelope JSON leaking to the UI). thinking is sourced from reasoning_content only, envelope-echo content unwrapped. Decompose + StepMapper side-calls unwrap envelope-wrapped replies via new StructuredDecisionAdapter.TryExtractAnswer. (2) EShellAgent heredoc-collapse guard (969cd36): a tool arg containing '<<' with NO newlines means the heredoc body was lost (small models collapse newlines; one-line zsh heredoc exits 0 and creates NOTHING — verified repro) → fail fast with guidance to ECodeEditor action=create; multiline heredocs still allowed. ECodeEditor create/patch verify-on-write: reports actual bytes on disk, write NOT verified = failure (no more model overclaiming). (3) BATCH APPROVALS FIXED (131a88c): ParallelToolExecutor was constructed without sessionOutput → every batch approval auto-DENIED at 0ms. Wired _out through. ToolRepeatTracker: Reset() per goal + Unrecord() for never-executed (DENIED) calls — user denials no longer count toward loop detection. Orchestrator envelope-echo guard on direct answers. (4) REMEMBER-DECISION APPROVALS (6eb8e0e): ApprovalScope (AllowOnce/AllowSession/Deny) through ISessionOutput.RequestApprovalScoped + IOutputListener.OnRequestApprovalScoped (default impls keep all listeners compiling); ToolPolicy session pattern approvals — EShellAgent → first command token, ECodeEditor → action, else first arg value (40 chars); session-scoped ONLY, never persisted, restart restores full gating. TUI prompt: "Approve? (y = once / a = always this session / n)". Live e2e verified both modes incl. long-conversation tool retention (12 turns then multi-file batch: approvals prompted, files verified on disk). (5) ECAssistantLLM CreateSession fix (edce9ab): unknown model ids resolved for fallback but the RAW id reached the session registry → 400 on every CreateSession → client permanently disabled structured decoding. Now passes the resolved model id. (6) ATTEMPTED AND REVERTED per Emre: non-blocking background planner (9278d0e → 5ffb099) — decompose on a background Task with a fold-in policy; fold-in verified live but deemed too risky; config keys removed. Exposed 3 independent bugs (all fixed in 131a88c/6eb8e0e). NEW: session-scoped remember-decision approvals (6eb8e0e): 'Approve? (y = once / a = always this session / n)' — ToolPolicy session pattern approvals (EShellAgent → first command token, ECodeEditor → action, else first arg), wired into single + batch paths; never persisted. Tests: adapter 11/11, tracker 10/10, policy 27/27. LDC regenerated, enforcement PASSED. NOT SHIPPED — rides the pending train (Core 12.9.12 → TUI → Console) when Emre says go.
**Status:** ✅ 0 errors, 0 warnings | LDC enforcement PASSED

**Updated:** 2026-09-22 (late afternoon — STATUS SPINNER (v14.10.1, TUI processing indicator): ISessionOutput.SetStatus default no-op + AgentSession dispatch to listeners via new IOutputListener.OnStatus default no-op; Orchestrator emits phases: Thinking before GenerateAsync, Running {tool} before single/batch ExecuteTool. TUI: StatusIndicator (UI/, braille frames + phase label + elapsed seconds, 90ms timer, EGuiConsole replace-last-line path) wired per-session via ConsoleUiRenderer statusCallback; spinner clears on any real OnOutput/OnStreamStart. TUI csproj whitelist updated. NOT SHIPPED — rides the pending train (Core 12.9.12 → TUI 12.9.13 → Console 1.0.10) when Emre says go. Previously: TRANSIENT-NULL FALLBACK FIX
...
Core handles multi-session orchestration, sub-agents, vector memory, self-correction, 11 built-in tools, and a TUI layer. The model, GPU layers, and KV cache live server-side; Core is a thin HTTP client plus agent logic.
...
└── Tools/                  # EToolBase + 11 built-in tools
