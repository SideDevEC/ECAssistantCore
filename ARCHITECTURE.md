...
**Updated:** 2026-09-22 (afternoon — JSON-ENVELOPE FALLBACK FIX (live regression from Emre screenshot): when the structured path falls back to text streaming, the model still emits the prompt-mandated {"thinking","answer"} envelope and ParseTextFallbackDecision surfaced it as RAW JSON to the user. Fix: TryParseJsonEnvelope (pure static, fence-tolerant) decodes the envelope in fallback — thinking → Reasoning, answer → clean AnswerText, toolcalls → parsed requests. JsonEnvelopeFallbackTests x5. Previously: CORRECTED RELEASE TRAIN SHIPPED (test-support-v12.9.11 → tui-v12.9.12 → console-v1.0.9, nuspec + SHA-bundle verified, all repos clean)
