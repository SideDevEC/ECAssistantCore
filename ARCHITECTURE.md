...
- **Tier operating profiles (v15):** system prompt + every tool block carry tier overlays — small tier: precision rules + worked examples + do-nots, one tool call per turn; large tier: autonomy profile (plan internally, batch, verify, minimal reporting). Tool-level tier prompts are enforced through `EToolBase.ToSystemPromptBlock(isLargeTier)` (hooks: `GetToolRulesForTier`/`GetToolExampleForTier`, defaulting to tier-agnostic overrides); the engine renders all registered tools for the active tier. system prompt carries a tier overlay — small tier: precision rules + worked examples + do-nots, one tool call per turn; large tier: autonomy profile (plan internally, batch, verify, minimal reporting). Reasoning effort (`llm_provider.reasoning_effort`, off/low/medium/high) flows to `reasoning_effort` on requests. Output-token budgets are NOT tier-dependent (config/hardware only — envelope budget removed).
