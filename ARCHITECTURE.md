...
**Updated:** 2026-09-22 (early AM — AskUser tool (v14.9, ambiguity-triggered checkpoints): model-driven EUserAskTool — model calls AskUser(question, options JSON array, optional default) when genuinely uncertain; orchestrator surfaces a real RequestChoice checkpoint via the session; user's pick flows back as tool output. Fallback ladder: user pick → declared default option → "proceed with best judgment" (autonomous when unattended). Registered in SessionBuilder when interaction.allow_user_ask (default true). Also fixed latent NRE: EAgentEngine structured-decoding branch dereferenced _config/_config.LlmProvider without null guards (CS8602/CS8604), InferenceParamsFactory.Create(_config) guarded, batch BuildCallSignature null-toolName guard. Interactive checkpoints (v14.9): ISessionOutput.RequestChoice(prompt, options) → 1-based index or null (null = proceed autonomously, non-breaking); IOutputListener.OnRequestChoice default-interface method (existing listeners keep compiling); AgentSession impl mirrors RequestApprovalAsync (immediate listener + bounded 60s wait); TUI ConsoleUiRenderer/AppController.PromptChoice (numbered list, Enter=cancel → autonomous); TestSupport TestSessionOutput prompts via GUI; Orchestrator hook #1: plan confirmation after decomposition when config interaction.confirm_plan=true (default false — autonomous unchanged): [Execute plan / Answer directly / Proceed without plan]; InteractionConfig section added — harness optimizations v14.9: (1) ToolRepeatTracker long-range loop detection in AgentOrchestrator — 2nd identical repeat or A→B→A→B alternation injects a redirect nudge, 3+ identical repeats stops the run (TurnsExhausted), batch path records signatures too; (2) tool-aware compaction — ContextWindow.TrimStaleToolOutputs(keepRecent) stubs older tool outputs (context_management.keep_recent_tool_outputs, default 3) instead of dropping all but the last; server-side toolset pinning in ECAssistantLLM (session tools_hash + deterministic KV reset on mismatch) — ServerLauncher: `--port` override now omitted when port <= 0; passing 0 crashed the server with "Invalid port in prefix." — server then falls back to the port in llm-server.json, default 48217. Regression tests added, ServerLauncherResolveTests 9/9)
