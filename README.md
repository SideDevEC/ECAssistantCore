...
- **Agent-decided specialist handoffs (`EHandoff`)** — delegate the entire remaining task to an ephemeral specialist with its own persona, tool subset, and *execution contract*: specialists get a task-execution decision frame (no "maybe it's just small talk" escape hatch), and the **delegating model itself** opts the specialist into per-task execution modes via the same grammar-constrained envelope — `sampling:"greedy"` (temperature 0 → native greedy chain, for deterministic deliverables like echo/fixed-format output) and `thinking:"off"` (Qwen3 `/no_think` soft switch — skips the think block server-side: less drift, ~30-40% fewer tokens on simple tasks). The specialist runs isolated, its answer becomes the final answer, then it is disposed. No semantic heuristics anywhere — the model declares *how* to run its specialist at delegation time.
